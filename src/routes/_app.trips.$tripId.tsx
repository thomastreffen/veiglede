    ...
    <div className="py-4">
      ...
      <section className={`mt-4 relative overflow-hidden rounded-3xl border border-border bg-gradient-to-br ${COVERS[trip.cover as CoverKey]} p-5 md:p-8`}>
        ...
        <div className="relative">
          ...
          {trip.inspiredByDisplayName && (
            <p className="mt-2 inline-flex items-center gap-1.5 rounded-full border border-primary/40 bg-primary/10 px-3 py-1 text-[11px] uppercase tracking-wider text-primary">
              ✦ Inspirert av {trip.inspiredByDisplayName}
            </p>
          )}
      ...
      <section className="mt-4 grid grid-cols-2 gap-3">
        <Link to="/trips/$tripId/roadbook" params={{ tripId }}
          className="inline-flex items-center justify-center gap-2 rounded-2xl bg-primary px-5 py-3.5 text-sm font-bold uppercase tracking-wider text-primary-foreground hover:brightness-110 whitespace-nowrap">
      ...
      <section id="track" className="mt-4 scroll-mt-24">
        <TripTracker tripId={tripId} tripStops={tripStops} vehicleLabel={`${v.emoji} ${vehicleDisplay}`} liveSession={liveSession} />
      </section>
      ...
      <section id="practical" className="mt-10 rounded-2xl border border-border bg-surface p-5 scroll-mt-24">
        ...
        <Link to="/trips/$tripId/roadbook" params={{ tripId }}
          className="mt-5 w-full inline-flex items-center justify-center gap-2 rounded-2xl bg-primary px-5 py-3.5 text-sm font-bold uppercase tracking-wider text-primary-foreground hover:brightness-110 whitespace-nowrap">
          <BookOpen className="h-4 w-4" /> {td.openRoadbook}
        </Link>
      </section>
              ...
              <div className="flex items-stretch">
                <Link to="/trips/$tripId/stops/$stopId" params={{ tripId, stopId: stop.id }} className="flex flex-1 items-start gap-3 p-4 hover:bg-surface-2/60 transition-colors min-w-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {stop.type === "lodging" ? <BookingInfo booking={stop.booking ?? {}} /> : null}
